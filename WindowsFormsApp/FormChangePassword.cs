...
namespace WindowsFormsApp
{
    public partial class FormChangePassword : Form
    {
        string matKhau, ma;
        bool check;
        public FormChangePassword(string matKhau, bool check, string ma)
        {
            InitializeComponent();
        ...
        private void guna2Button1_Click_1(object sender, EventArgs e)
        {
            ...
            else
            {
                if (check)
                {
                    CustomerBUS.Intance.doiMatKhau(ma, txtMoi.Text);
                }
                else
                {
                    EmployeeBUS.Intance.doiMatKhau(ma, txtMoi.Text);
                }
